...
import { useGameMutation, useGameMutationCallback } from '@/hooks/games/use-game-request';
...
export function EasyCraps({gameSession}: EasyCrapsProps) {
  ...
  const {data: game, isLoading: gameLoading, refetch: refetchGame} = useQuery(crpc.games.craps.easy.getSession.queryOptions(gameSession));
  
  const betPassline = useGameMutationCallback(gameSession, crpc.games.craps.easy.betPassline.mutationOptions());
  const {data: rollResult, isSuccess: rollSucceeded, isPending: rollInProgress, mutate: doRoll} = useGameMutation(gameSession, crpc.games.craps.easy.roll.mutationOptions({
    onSuccess: async () => {
      await refetchGame();
    }
  }));
  ...
  return (
    <>
      ...
      <div className="absolute top-0 lg:-top-10 flex flex-col items-center">
        <div
          onClick={() => doRoll()}
          className="flex flex-row items-center w-fit h-fit bg-none"
        >
          {!rollInProgress && !rollSucceeded ? "Roll!" : null}
          {rollInProgress ? "rolling..." : null}
          {rollSucceeded ? (
            rollResult.dice.map((diceVal,i) => <DiceComponent key={i} value={diceVal} size={100} />)
          ) : null}
        </div>
      </div>
            ...
            <div className="flex flex-col items-center gap-1 p-1">
              ...
              {lastRoll ? (
                <div className="flex flex-row items-center gap-1">
                  {lastRoll.map((diceVal,i) => <DiceComponent key={i} value={diceVal} size={30} />)}
                </div>
              ) : null}
            </div>
            <div className="flex flex-row gap-4 overflow-x-scroll w-full">
              {game?.rollHistory.slice(1).map((roll, i) => 
                <div key={i} className="flex flex-row items-center gap-1">
                  {roll.map((diceVal,j) => <DiceComponent key={j} value={diceVal} size={24} />)}
                </div>
              )}
            </div>
          </div>
