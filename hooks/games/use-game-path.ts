...
export function useGamePath(): GamePath {
  ...
  if (!path.startsWith("/games/")) throw new Error(`useGamePath must be called from a /games route!`);
  const gamePath = path.split('/').slice(2)
  const gamePair = getGameByPath(gamePath);
  if (!gamePair) throw new Error(`useGamePath must be called from a /games route!`);
