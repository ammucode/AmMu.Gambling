...
type CompForAGameComponent<Comp extends aGameComponent> =
  Parameters<Comp>[0]['game'] extends RootGameWithSubs ? Optional<Comp> : Comp;
...
function withIcon<Comp extends aGameComponent, Icon extends LucideIcon>(
  component: Comp,
  icon?: Icon
) {
...
type GameComponentsDef = {
  [Path in GamePathString]: Path extends `${string}/${string}`
    ? [GameCompWithIcon<RootGameComponent>, GameCompWithIcon<SubGameComponent>]
    : [GameCompWithIcon<GameComponent>, undefined];
};
...
export type GameComponentDef = Exclude<
  GameComponentDefs[keyof GameComponentDefs][number],
  undefined
>;
